function [ output_args ] = sampleFgPatches( params )
scales = [3 4 5 6];
th = 0.3;

mkdir(fullfile(params.path.data, 'smc', 'patches_fg'));
...
for i=1:length(trainingImgs)
    ...
    for s=1:length(scales)
        ...
        for j=1:min(2,length(idx))
            ...
            %output
            k=k+1;
            imwrite(patchImg, fullfile(params.path.data, 'smc', 'patches_fg', sprintf('%d.jpg', k)));
            save(fullfile(params.path.data, 'smc', 'patches_fg', sprintf('%5d.mat', k)), 'patchHog', 'patchItti', 'gtSal', 's');
        end
    end
    fprintf('.');
end
